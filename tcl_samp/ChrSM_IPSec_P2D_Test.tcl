#***************************************************************
#
#  IxChariot API SDK              File: ChrSM_IPSec_P2D_Test.tcl
#
...
#  over IPSec tunnels between ixia port and DUT, then saves the 
#  test to a file. The DUT needs to be configured correctly
...
set testFile "ChrSM_IPSec_P2D_Test.tst"
set pairCount 1
...
set e1_mgmt "192.168.4.170;1;3"
set e2_mgmt "192.168.4.170;1;4"
set e1Mgmt "192.168.4.170 / 01 / 03"
set e2Mgmt "192.168.4.170 / 01 / 04"
...
# Set up Emulated Router Plugin
       ...
       set erpObj [$pgObj stack childrenList Get 0]
        ...
        $erpObj ipAddress Set "172.21.254.10"
        $erpObj incrementBy Set "0.0.0.1"
        $erpObj gatewayAddress Set "172.21.254.1"
        $erpObj prefix Set "16"
...
# Set up IP Plugin 
       ...
       $iprObj ipAddress Set 172.21.254.3
...
# Set up IPSec Plugin
    ...
    set isnObj [$ipsObj ipsecNetwork Get 0]
       ...
       $isnObj peerPublicIP Set "172.21.254.1"     
...
# Set up Emulated Router Plugin
    ...
    set erpObj [$pgObj1 stack childrenList Get 0]
        $erpObj ipType Set "IPv4"
        $erpObj ipAddress Set "172.22.254.10"
        $erpObj incrementBy Set "0.0.0.1"
        $erpObj gatewayAddress Set "172.22.254.1"
        $erpObj prefix Set "16"
