#***************************************************************
#
#  IxChariot API SDK              File: ChrSM_PPP_P2D_Test.tcl
#
...
#  using PPP Plugin for port to DUT scenario.
#
#  The DUT needs to be configured correctly for the script to
#  work.
#
#***************************************************************
...
set testFile "ChrSM_PPP_P2D_Test.tst"
set pairCount 1
...
set CHASSIS1_IP 192.168.6.163
set e1_mgmt "192.168.6.163;4;1"
set e2_mgmt "192.168.6.163;4;2"
set e1Mgmt "192.168.6.163 / 04 / 01"
set e2Mgmt "192.168.6.163 / 04 / 02"
...
# Set up first portgroup
    ...
    set pg_idx [expr [$::myTest portGroupList Size] -2]
       set pgClientObj [$::myTest portGroupList Get $pg_idx] 
       $pgClientObj name Set "Client Network"
       $pgClientObj portList AddTail $portSpec

# Set up Ethernet Stack on "My first port group" - pgObj
       $pgClientObj _Instantiate stack "EthernetPlugin"
       $pgClientObj stack enabled Set true
       $pgClientObj stack mac Set "00:00:00:00:01:01"

       # Add PPP Plugin
    $pgClientObj stack childrenList AddTail -itemtype "PppoxPlugin"
    set pppoxClientObj [$pgClientObj stack childrenList Get 0]
        $pppoxClientObj comment Set "pppox_client_plugin"
        $pppoxClientObj enabled Set "true"
        $pppoxClientObj role Set "client"
        $pppoxClientObj setupRate Set "100"
        $pppoxClientObj enableThrottling Set "true"
        $pppoxClientObj maxOutstandingSessions Set "50"
        $pppoxClientObj teardownRate Set "100"
        $pppoxClientObj numSessions Set "1"
...
# Set up second portgroup
       ...
       set pgServerObj [$::myTest portGroupList Get $pg_idx] 
       $pgServerObj name Set "Server Network"
       $pgServerObj portList AddTail $portSpec

# Set up Ethernet Stack 
       $pgServerObj _Instantiate stack "EthernetPlugin"
       $pgServerObj stack enabled Set true
       $pgServerObj stack mac Set "00:00:00:00:02:02"
# Set up vlan range
       $pgServerObj stack vlanRangeList AddTail
       set vlanRObj [$pgServerObj stack vlanRangeList Get 0]
       $vlanRObj enable Set true
       $vlanRObj firstId Set "3"

# Set up IP plugin on "Server Network"
       $pgServerObj stack childrenList AddTail -itemtype "IpV4V6Plugin"
       set ippObj [$pgServerObj stack childrenList Get 0]
       $ippObj rangeList AddTail
       set iprObj [$ippObj rangeList Get 0]
# Bind the ip range object to the vlan range above
       $iprObj _Bind vlanRange $vlanRObj
       $iprObj name Set "ip-2"
       $iprObj enabled Set true
       $iprObj ipType Set IPv4
       $iprObj ipAddress Set "31.0.10.1"
       $iprObj prefix Set 8
       $iprObj count Set 1
       $iprObj gatewayAddress Set "31.0.0.1"
