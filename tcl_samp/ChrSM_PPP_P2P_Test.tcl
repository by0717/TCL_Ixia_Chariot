#***************************************************************
#
#  IxChariot API SDK              File: ChrSM_PPP_P2P_Test.tcl
#
...
#  using PPP Plugin for port to port, then saves the test to a file.
#
...
set testFile "ChrSM_PPP_P2P_Test.tst"
set pairCount 1
...
# Set up first portgroup
    ...
    set pg_idx [expr [$::myTest portGroupList Size] -2]
       ...
       $pgClientObj name Set "My First Port Group"
...
# Set up Ethernet Stack on "My first port group" - pgObj
    ...
    set pppoxClientObj [$pgClientObj stack childrenList Get 0]
        $pppoxClientObj comment Set "my_pppox_client_plugin"
        ...
        $pppoxClientObj numSessions Set "10"
...
# Set up second portgroup
       ...
       $pgServerObj name Set "My Second Port Group"
       $pgServerObj portList AddTail $portSpec

# Set up Ethernet Stack on "My Second Port Group" 
...
# Set up IP plugin on "My Second Port Group" 
    $pgServerObj stack childrenList AddTail -itemtype "PppoxPlugin"
    set pppoxServerObj [$pgServerObj stack childrenList Get 0]
        $pppoxServerObj comment Set "my_pppox_server_plugin"
        $pppoxServerObj enabled Set "true"
        $pppoxServerObj role Set "server"
        $pppoxServerObj setupRate Set "100"
        $pppoxServerObj enableThrottling Set "true"
        $pppoxServerObj maxOutstandingSessions Set "50"
        $pppoxServerObj teardownRate Set "100"
        $pppoxServerObj numSessions Set "10"    
...
for {set index 0} {$index < $pairCount} {incr index} {
  ...
  chrPair set $pair E1_ADDR "My First Port Group" 
  chrPair set $pair E2_ADDR "My Second Port Group"
