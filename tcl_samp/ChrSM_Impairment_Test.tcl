#***************************************************************
#
#  IxChariot API SDK              File: ChrSM_Impairment_Test.tcl
#
...
#  using impairment Plugin, then saves the chariot test to a file.
#
...
set testFile "ChrSM_Impairment_Test.tst"
set pairCount 1
set e1Addrs "172.16.1.1"
set e2Addrs "172.16.2.1"

set protocols "TCP"
set scripts "c:/Program Files/Ixia/IxChariot/Scripts/Throughput.scr" 
set timeout 5
set maxWait 180
set logFile "pairsTest.log"

set CHASSIS1_IP 192.168.4.170
set e1_mgmt "192.168.4.170;1;1"
set e2_mgmt "192.168.4.170;1;2"
set e1Mgmt "192.168.4.170 / 01 / 01"
set e2Mgmt "192.168.4.170 / 01 / 02"
...
# Set up Impairment plugin
    $pgObj stack childrenList AddTail -itemtype "ImpairmentPlugin"
    set impObj [$pgObj stack childrenList Get 0]
        # To add pkt loss
        $impObj addDrop Set true        
        #percentage of pkts to drop
        $impObj drop Set 3              

        # to add latency
#       $impObj addDelay Set true  
        # for pkt reordering
#       $impObj addReorder Set true 
        # to set add pkt duplicates
#       $impObj addDuplicate Set true 
        # to limit the rate of traffic
#       $impObj addBandwidth Set true 
        # for randomizing the seed for impairment
#       $impObj randomizeSeed Set true 
        # delay in ms to be added to each pkt
#       $impObj delay Set 10            
        # impairment applied only to this ipAddress
#       $impObj destinationIp Set "$gateway1" 
        # to add impair to all interfaces in underlying eth layer
#       $impObj global Set true  

# Set up IP plugin on "My first port group" - pgObj
       $impObj childrenList AddTail -itemtype "IpV4V6Plugin"
       set ippObj [$impObj childrenList Get 0]
