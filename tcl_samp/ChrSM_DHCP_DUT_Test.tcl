#***************************************************************
#
#  IxChariot API SDK              File: ChrSM_DHCP_DUT_Test.tcl
#
...
#  using IP Plugin as a DHCP client on both ends. For the script to work 
#  DHCP server needs to be enabled on the DUT.
#
...
set testFile "ChrSM_DHCP_DUT_Test.tst"
set pairCount 1
...
# Set up first portgroup
    ...
    set pg_idx [expr [$::myTest portGroupList Size] -2]
       ...
       $pgObj name Set "Client Network1"
...
# Set up IP plugin on "My first port group" - pgObj
       ...
       $iprObj addressAllocationMechanism Set "dhcp"
       $iprObj _Instantiate dhcpSettings "DhcpSettings"
       $iprObj dhcpSettings timeOut Set 20
...
# Set up second portgroup
       ...
       $pgObj1 name Set "Client Network2"
...
for {set index 0} {$index < $pairCount} {incr index} {
  ...
  chrPair set $pair E1_ADDR "Client Network1"
  chrPair set $pair E2_ADDR "Client Network2"
